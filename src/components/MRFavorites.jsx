import "../css/MRFavorites.css";
import { data } from "../js/MypageRepository.js"

function MypageRepositoryBtn({ btn }) {
    return (
        <button className={`mypagerepository-${btn.class}`}>
            <img src={btn.src} alt="" />
            <p>{btn.text}</p>
        </button>
    )
}

function MRFavorites() {
    return (
        <section>
            <div className="mrf">
                <div className="mrf-inner">
                    <div className="mrf-header">
                        <div className="mrf-title">
                            <h2>내 자료함</h2>
                            <img src='/images/mypagerepository.png' alt="" />
                        </div>
                        <div className="search">
                            <input
                                type="text"
                                placeholder="검색어를 입력해주세요"
                            // value={keyword}
                            // onChange={(e) => setKeyword(e.target.value)}
                            />
                            <button type="button">
                                <img src="/images/icon/search1.png" alt="검색" />
                            </button>
                        </div>
                    </div>

                    <div className="mrf-contents">
                        <div className="mrf-inventory">
                            {data.map((item, index) => (
                                <MypageRepositoryBtn key={index} btn={item} />
                            ))}
                            <div className="mrf-button">
                                <div className="mrf-button-span">
                                    <div className="Storage-bar"><div className="Storage-ber-used"></div></div>
                                </div>
                                <p className="Storage-capacity">43.2GB/100GB</p>
                                <button>추가 저장공간 구매</button>
                            </div>
                        </div>
                        <div className="mrf-contents-inner">
                            <div className="mrf-main">
                                <div className="mrf-main-header">
                                    <img src="/images/icon/aroow.png" alt="" />
                                    <p>즐겨찾기</p>
                                    <button className="mr-add">
                                        <img src="/images/icon/Plus.png" alt="" />
                                    </button>
                                </div>
                            </div>
                            <div className="mrf-file-list">
                                <div className="mrf-file-name">
                                    <p>이름</p>
                                    <p>날짜</p>
                                    <p>형식</p>
                                    <p>크기</p>
                                </div>
                                <div className="mrf-file-suggestion">
                                    {/* <input type="checkbox" /> */}
                                    <div className="mrf-file-gather">
                                        <button><img src="/images/icon/img.png" alt="" /></button>
                                        <p>자료구조 노트필기</p>
                                    </div>
                                    <p>12월 1일 12:10</p>
                                    <p>.jpg</p  >
                                    <p>20GB</p>
                                    <div className="mrf-file-img-gather">
                                        <button className="mrf-reroll">
                                            <img src="/images/icon/reroll.png" alt="" />
                                        </button>
                                        <button>
                                            <div className="mrf-ooo">
                                                <span>•</span>
                                                <span>•</span>
                                                <span>•</span>
                                            </div>
                                        </button>
                                    </div>
                                </div>
                            </div>
                            <div className="mrf-file-list">
                                <div className="mrf-file-name"></div>
                                <div className="mrf-file-suggestion">
                                    {/* <input type="checkbox" /> */}
                                    <div className="mrf-file-gather">
                                        <button><img src="/images/icon/folder.png" alt="" /></button>
                                        <p>사회</p>
                                    </div>
                                    <p>12월 1일 12:10</p>
                                    <p>.jpg</p  >
                                    <p>20GB</p>
                                    <div className="mrf-file-img-gather">
                                        <button className="mrf-reroll">
                                            <img src="/images/icon/reroll.png" alt="" />
                                        </button>
                                        <button>
                                            <div className="mrf-ooo">
                                                <span>•</span>
                                                <span>•</span>
                                                <span>•</span>
                                            </div>
                                        </button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    );
}

export default MRFavorites;